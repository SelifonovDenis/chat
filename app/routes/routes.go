...
func (_ tCChat) ChatSocket(
		ws interface{},
		) string {
	args := make(map[string]string)
	
	revel.Unbind(args, "ws", ws)
	return revel.MainRouter.Reverse("CChat.ChatSocket", args).URL
}
...
func (_ tCUser) GetUser(
		) string {
	args := make(map[string]string)
	
	return revel.MainRouter.Reverse("CUser.GetUser", args).URL
}
